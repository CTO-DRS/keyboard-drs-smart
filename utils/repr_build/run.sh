...
IMAGE_NAME="drs-repr-build"
IMAGE_TAG="default"
IMAGE="$IMAGE_NAME:$IMAGE_TAG"

USER_NAME="runner"
REPO_MOUNT="/home/$USER_NAME/keyboard-drs-smart"
GRADLE_CACHE_VOLUME_NAME="drs-repr-build-gradle-cache"
...
docker_run() {
  docker_run_with_env() {
    if [ "$DRSSEC_SIGNING_ENABLED" = "1" ]; then
      local num_missing=0
      ensure_defined() {
        [ -n "${!1}" ] || {
      ...
      ensure_defined "DRSSEC_KEYFILE"
      ensure_defined "DRSSEC_KEYALIAS"
      ensure_defined "DRSSEC_KS_PASS"
      ensure_defined "DRSSEC_KEY_PASS"
      [ $num_missing -eq 0 ] || exit 1
      [ -f "$DRSSEC_KEYFILE" ] || {
        echo "fatal: DRSSEC_KEYFILE provided, but file '$DRSSEC_KEYFILE' does not exist"
        exit 1
      }
      local KEYFILE_IN_CONTAINER="/home/$USER_NAME/keystore.jks"
      docker run \
        -e DRSSEC_SIGNING_ENABLED=1 \
        -e DRSSEC_KEYFILE="$KEYFILE_IN_CONTAINER" \
        -e DRSSEC_KEYALIAS="$DRSSEC_KEYALIAS" \
        -e DRSSEC_KS_PASS="$DRSSEC_KS_PASS" \
        -e DRSSEC_KEY_PASS="$DRSSEC_KEY_PASS" \
        -v "$DRSSEC_KEYFILE:$KEYFILE_IN_CONTAINER:ro" \
        "$@"
    else
      docker run "$@"
    fi
