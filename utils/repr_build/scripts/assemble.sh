...
sign_and_write_apk() {
  ...
  "$APKSIGNER_PATH" sign -v \
    --ks "${DRSSEC_KEYFILE}" \
    --ks-key-alias "$DRSSEC_KEYALIAS" \
    --ks-pass "pass:$DRSSEC_KS_PASS" \
    --key-pass "pass:$DRSSEC_KEY_PASS" \
    --out "$APK_PATH" \
    "$APK_PATH"
}
...
sign_and_write_aab() {
  ...
  "$JARSIGNER_PATH" -sigalg SHA256withRSA -digestalg SHA256 \
    -keystore "${DRSSEC_KEYFILE}" \
    -storepass "$DRSSEC_KS_PASS" \
    -keypass "$DRSSEC_KEY_PASS" \
    -signedjar "$AAB_PATH" \
    "$AAB_PATH" "$DRSSEC_KEYALIAS"
}
...
build() {
  ...
  if [ "$TRACK" = "$TRACK_DEBUG" ]; then
    ...
    OUTPUT_FILE_PATH="$OUTPUT_VERSION_DIR/drs-keyboard-$BUILD_VERSION_NAME.$TYPE"
  else
    OUTPUT_VERSION_DIR="$OUTPUT_DIR/$BUILD_VERSION_NAME-$TRACK"
    OUTPUT_FILE_PATH="$OUTPUT_VERSION_DIR/drs-keyboard-$BUILD_VERSION_NAME-$TRACK.$TYPE"
  fi
