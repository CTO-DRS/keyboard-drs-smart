...
import org.drs.lib.android.UriSerializer
...
data class ClipboardItem @OptIn(ExperimentalSerializationApi::class) constructor(
    ...
    companion object {
        ...
        const val DRS_CLIP_LABEL = "drs/clipboard_item"
    ...
    fun toClipData(context: Context): ClipData {
        return when (type) {
            ItemType.TEXT -> {
                ClipData.newPlainText(DRS_CLIP_LABEL, text)
            }
            ItemType.IMAGE, ItemType.VIDEO -> {
                ClipData.newUri(context.contentResolver, DRS_CLIP_LABEL, uri)
            }
        }
    }
