...
package com.drs.smartkeyboard.app.devtools
...
import com.drs.smartkeyboard.ime.keyboard.CachedLayout
import com.drs.smartkeyboard.ime.keyboard.DebugLayoutComputationResult
...
import org.drs.lib.snygg.SnyggMissingSchemaException

private val CardBackground = Color.Black.copy(0.6f)
private val DateFormat = SimpleDateFormat("yyyy-MM-dd'T'HH-mm-ss", DrsLocale.default().base)
    ...
    DevtoolsOverlayBox(title = "Failed to load stylesheet, fell back to base style") {
        ...
        if (cause is SnyggMissingSchemaException) {
            DevtoolsSubGroup(title = "Explanation") {
                DevtoolsText(
                    text = """
                    It appears you’re trying to load a theme designed for DRS Smart Keyboard v0.4 (Snygg v1), which isn’t compatible with the latest release using Snygg v2.
