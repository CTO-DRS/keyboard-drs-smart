...
import com.drs.smartkeyboard.lib.DrsLocale.Companion.default
import java.util.*
 ...
 *
 * To construct a DrsLocale, use one of the many from() methods provided.
 *
 * @see java.util.Locale
 */
@Serializable(with = DrsLocale.Serializer::class)
class DrsLocale private constructor(val base: Locale) {
    companion object {
        ...
        /**
         * Wraps a [java.util.Locale] and returns the [DrsLocale].
         *
         * @return The wrapped locale.
         */
        fun from(javaLocale: Locale) = DrsLocale(javaLocale)

        /**
         * Constructs a new [DrsLocale] with given [language].
         *
         * @param language A two-letter language code.
         *
         * @return A new [DrsLocale].
         */
        fun from(language: String) = from(Locale(language))

        /**
         * Constructs a new [DrsLocale] with given [language] and [country].
         *
         * @param language A two-letter language code.
        ...
        /**
         * Constructs a new [DrsLocale] with given [language], [country] and [variant].
         *
         * @param language A two-letter language code.
         * @param country A two-letter country code.
        ...
        /**
         * Constructs a new [DrsLocale] from given [str].
         *
         ...
         */
        fun default() = DrsLocale(Locale.getDefault())
         ...
         */
        fun installedSystemLocales(): List<DrsLocale> = Locale.getAvailableLocales().map { from(it) }

        /**
         * Returns a list of all installed locales and custom locales.
         *
         */
        fun extendedAvailableLocales(context: Context): List<DrsLocale> {
     ...
     */
    fun displayLanguage(locale: DrsLocale = default()): String {
     ...
     */
    fun displayCountry(locale: DrsLocale = default()): String = base.getDisplayCountry(locale.base)
     ...
     */
    fun displayVariant(locale: DrsLocale = default()): String = base.getDisplayVariant(locale.base)
     ...
     *  [language], [country] and [variant] are not specified.
     */
    fun displayName(locale: DrsLocale = default()) = buildString {
     ...
     */
    override fun toString() = "DrsLocale { l=${base.language} c=${base.country} v=${base.variant} }"
    ...
    /**
     * The JSON (de)serializer for DrsLocale.
     */
    class Serializer : KSerializer<DrsLocale> {
        override val descriptor: SerialDescriptor =
            PrimitiveSerialDescriptor("DrsLocale", PrimitiveKind.STRING)

        override fun serialize(encoder: Encoder, value: DrsLocale) {
            encoder.encodeString(value.languageTag())
        }

        override fun deserialize(decoder: Decoder): DrsLocale {
...
inline fun String.lowercase(locale: DrsLocale): String = this.lowercase(locale.base)

@Suppress("NOTHING_TO_INLINE")
inline fun String.uppercase(locale: DrsLocale): String = this.uppercase(locale.base)

@Suppress("NOTHING_TO_INLINE")
inline fun String.titlecase(locale: DrsLocale = DrsLocale.ROOT): String {
