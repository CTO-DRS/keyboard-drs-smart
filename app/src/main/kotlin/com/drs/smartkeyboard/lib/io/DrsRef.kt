 ...
 * DRS Smart Keyboard's app user interface screens, APK assets, cache and internal
 * storage, external resources provided to DRS Smart Keyboard via content URIs, as
 * well as hyperlinks.
 *
 ...
 */
@Serializable(with = DrsRef.Serializer::class)
@JvmInline
value class DrsRef private constructor(val uri: Uri) {
    companion object {
        @VisibleForTesting(otherwise = VisibleForTesting.PRIVATE)
        internal const val SCHEME_DRS = "drs-keyboard"
        ...
        /**
         * Constructs a new [DrsRef] pointing to a UI screen within the app
         * user interface.
         *
        ...
        /**
         * Constructs a new [DrsRef] pointing to a resource within the
         * DRS Smart Keyboard APK assets.
         *
        ...
        /**
         * Constructs a new [DrsRef] pointing to a resource within the
         * cache storage of DRS Smart Keyboard.
         *
        ...
        /**
         * Constructs a new [DrsRef] pointing to a resource within the
         * internal storage of DRS Smart Keyboard.
         *
         ...
         * destination, regardless of within DRS Smart Keyboard or not.
         *
         * @param uri The destination, denoted by a system URI format.
         *
         * @return The newly constructed reference.
         */
        fun from(uri: Uri) = DrsRef(uri)
         ...
         * point to any destination, regardless of within DRS Smart Keyboard or not.
         *
     ...
     */
    val isAppUi: Boolean
        get() = uri.scheme == SCHEME_DRS && uri.authority == AUTHORITY_APP_UI

    /**
     * True if the scheme and authority indicates a reference to a DRS Smart Keyboard APK asset
     * resource, false otherwise.
     */
    val isAssets: Boolean
        get() = uri.scheme == SCHEME_DRS && uri.authority == AUTHORITY_ASSETS

    /**
     * True if the scheme indicates a reference to a DRS Smart Keyboard cache
     * resource, false otherwise.
     */
    val isCache: Boolean
        get() = uri.scheme == SCHEME_DRS && uri.authority == AUTHORITY_CACHE

    /**
     * True if the scheme indicates a reference to a DRS Smart Keyboard internal
     * storage resource, false otherwise.
     */
    val isInternal: Boolean
        get() = uri.scheme == SCHEME_DRS && uri.authority == AUTHORITY_INTERNAL
    ...
    /**
     * Returns if this URI contains data for all valid parts of a DrsRef.
     */
    val isValid: Boolean
     ...
     *
     * @param assets The lambda to run when the reference points to the DRS Smart Keyboard
     *  screen UI resources. Defaults to do nothing.
     * @param assets The lambda to run when the reference points to the DRS Smart Keyboard
     *  APK assets. Defaults to do nothing.
     * @param cache The lambda to run when the reference points to the DRS Smart Keyboard
     *  cache resources. Defaults to do nothing.
     * @param internal The lambda to run when the reference points to the DRS Smart Keyboard
     ...
     */
    fun whenSchemeIs(
        appUi: (ref: DrsRef) -> Unit = { /* Do nothing */ },
        assets: (ref: DrsRef) -> Unit = { /* Do nothing */ },
        cache: (ref: DrsRef) -> Unit = { /* Do nothing */ },
        internal: (ref: DrsRef) -> Unit = { /* Do nothing */ },
        external: (ref: DrsRef) -> Unit = { /* Do nothing */ }
    ) {
        contract {
    ...
    object Serializer : PreferenceSerializer<DrsRef>, KSerializer<DrsRef> {
        override val descriptor = PrimitiveSerialDescriptor("DrsRef", PrimitiveKind.STRING)

        override fun serialize(value: DrsRef): String {
            return value.toString()
        }

        override fun serialize(encoder: Encoder, value: DrsRef) {
            encoder.encodeString(value.toString())
        }

        override fun deserialize(value: String): DrsRef {
            return from(value)
        }

        override fun deserialize(decoder: Decoder): DrsRef {
